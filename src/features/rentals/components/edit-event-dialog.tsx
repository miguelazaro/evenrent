...
export function EditEventDialog({ event, open, onOpenChange }: EditEventDialogProps) {
  ...
  useEffect(() => {
    if (event) {
      form.reset({
        ...
        deliveryDate: new Date(event.deliveryDate),
        pickupDate: event.pickupDate ? new Date(event.pickupDate) : undefined,
        notes: event.notes || '',
        status: event.status,
      });
      ...
      if (event.items && Array.isArray(event.items)) {
        setItems(
          event.items.map((ri: any) => ({
            inventoryItemId: ri.inventoryItemId,
            name: ri.inventoryItem?.name ?? ri.inventoryItemId,
  ...
  const updateMutation = trpc.rental.update.useMutation({
    onSuccess: () => {
